...
int main() {
    ...
    //p = 0x5ffe94; // using the value from my Watch window
    ...
    *q  = *p;      
    ...
    /*
    Fails because sal is a const int, and int* cannot be assigned to const int.
    pI = &sal;  
    */
    ...
    //pcI = &joe;
    /*
    Fails because since pcI is const, it is not modifiable, cannot change value
    *pcI = 234;  
    */ 
    ...
    */   
    ...
    /*
    cpI is not initialized here
    int* const cpI;
    */
   
    //int* const cpI(&joe);

    //int* const cannot initialize a const int
    //int* const cpI(&sal);

    //cpI is not modifiable, cannot change it
    //cpI = &joe;  
    ...
    //Nothing is modifiabe, so cannot change anything
    //*cpcI = 234;  
    //cpcI = &sal;  
    //*cpcI = 7623; 
...
for (size_t ndx = 0; ndx < colours.size(); ++ndx) {            
    ...
    amp.playMusic();
    //attaching speakers to complete system
